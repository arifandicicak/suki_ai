...
export default function SukiMascot({ currentUnitTitle, userExp }: SukiMascotProps) {
  ...
  // Drawer Dimensions & Calculations to prevent clipping
  const drawerWidth = window.innerWidth < 640 ? 330 : 380;
  const drawerHeight = window.innerWidth < 640 ? 400 : 450;
    ...
    <>
      ...
      >
        ...
        >
          ...
          <div className="text-2xl sm:text-3xl filter drop-shadow select-none">
            {activeRoleplayChar ? "🔮" : "🌸"}
          </div>
      ...
      {isOpen && (
        <div 
          ...
          className="bg-[#FCFAF2] border-2 border-stone-950 rounded-3xl shadow-2xl p-4 flex flex-col max-h-[70vh] sm:max-h-[60vh] overflow-hidden select-text animate-fade-in animate-slide-up"
        >
          
          {/* Suki Panel Header */}
          <div className="flex items-center justify-between border-b-2 border-stone-200 pb-3 select-none">
            <div className="flex items-center gap-2">
              <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-emerald-400 to-green-500 flex items-center justify-center text-white text-lg">
                {activeRoleplayChar ? "🔮" : "🌸"}
              </div>
              <div className="max-w-[180px] sm:max-w-[220px]">
                <h4 className="font-sans font-black text-xs md:text-sm text-stone-900 flex items-center gap-1 truncate">
            ...
            <button
              ...
              className="text-stone-400 hover:text-stone-700 p-1 rounded-full hover:bg-stone-100 cursor-pointer shrink-0"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto py-3 space-y-3.5 sm:max-h-[300px] scrollbar-thin">
            {messages.map((m, idx) => {
              const isAI = m.role === "assistant";
              return (
                <div key={idx} className={`flex gap-2 ${isAI ? "justify-start" : "justify-end"}`}>
                  {isAI && (
                    <div className="w-7 h-7 rounded-full bg-emerald-100 border border-emerald-200 shrink-0 flex items-center justify-center text-sm shadow-sm select-none">
                      {activeRoleplayChar ? "🔮" : "🌸"}
                    </div>
                  )}
                  ...
                  <div className={`max-w-[85%] rounded-2xl px-3.5 py-2.5 text-xs shadow-sm leading-relaxed ${
                    isAI 
                      ? "bg-stone-50 border border-stone-200 text-stone-800 rounded-tl-none pr-8"
            ...
            {loading && (
              <div className="flex gap-2 justify-start">
                <div className="w-7 h-7 rounded-full bg-emerald-100 flex items-center justify-center text-sm animate-spin">
                  {activeRoleplayChar ? "🔮" : "🌸"}
                </div>
                <div className="bg-stone-50 border border-stone-200 text-stone-700 rounded-2xl px-3.5 py-2.5 text-xs shadow-inner flex items-center gap-1.5">
            ...
            {errorMsg && (
              <p className="text-[10px] text-amber-700 italic text-center">{errorMsg}</p>
            )}
          ...
          <div className="py-2 border-t border-stone-150 flex flex-wrap items-center justify-between gap-1.5 select-none">
            <button
              onClick={handleRequestClue}
              type="button"
              className="py-1 px-2.5 bg-amber-55 hover:bg-amber-100 border border-amber-200 rounded-lg text-[9px] font-sans font-black text-amber-800 flex items-center gap-1 transition-all cursor-pointer"
            >
            ...
            {activeRoleplayChar && (
              <button
                ...
                type="button"
                className="bg-stone-100 hover:bg-stone-200 border border-stone-300 rounded-lg py-1 px-2.5 text-[9px] font-sans font-black text-stone-700 transition cursor-pointer"
              >
          ...
          {/* Chat user input bar */}
          <form onSubmit={handleSendMessage} className="flex gap-1.5 mt-1 border-t border-stone-150 pt-2 select-none">
            <input
              type="text"
              value={inputVal}
              onChange={(e) => setInputVal(e.target.value)}
              placeholder={activeRoleplayChar ? `Panggil ${activeRoleplayChar.character}...` : "Ketik pesan untuk Suki..."}
              className="flex-1 bg-stone-50 border border-stone-200 rounded-xl px-3 py-1.5 text-xs font-sans outline-none focus:bg-white focus:ring-1 focus:ring-emerald-500 text-stone-800"
            />
            <button
              type="submit"
              disabled={loading || !inputVal.trim()}
              className="bg-emerald-600 hover:bg-emerald-700 disabled:bg-stone-200 text-white p-2 rounded-xl transition-all cursor-pointer shrink-0"
            >
